...
| **Prisma Postgres** | — | Vercel 서버리스 환경에서 로컬 DB 사용 불가 → 클라우드 PostgreSQL 필수. Prisma ORM과 네이티브 통합 |
...
| 기술 | 검토 대안 | 선택하지 않은 이유 |
|------|-----------|-------------------|
| Database | PlanetScale, Neon, Supabase | Prisma Postgres는 Prisma ORM과 네이티브 통합되어 별도 설정 없이 사용 가능 |
...
이 스택은 타입 안전성(TypeScript + Prisma + Zod), 서버 중심 아키텍처(RSC + Server Actions), 클라우드 네이티브 배포(Vercel + Prisma Postgres) 세 축을 중심으로 구성되며, 각 기술이 서로의 강점을 보완한다.
